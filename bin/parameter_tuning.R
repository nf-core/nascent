...
print(paste("Number of transcripts_ranges before reduction:", length(exons_by_transcript)))

# Reduce exons to create transcript ranges
transcripts_ranges <- GenomicRanges::reduce(exons_by_transcript)
transcripts_ranges <- unlist(transcripts_ranges, use.names = FALSE)
...
mapping_df <- data.frame(
  transcript_id = names(exons_by_transcript),
  gene_id = vapply(exons_by_transcript, function(x) unique(x$gene_id)[1], character(1)),
  stringsAsFactors = FALSE
)

# Check for length mismatch
if (nrow(mapping_df) != length(transcripts_ranges)) {
  stop("Length mismatch between mapping_df and transcripts_ranges. Please check the reduction step.")
}

# Assign metadata
transcripts_ranges$transcript_id <- mapping_df$transcript_id
transcripts_ranges$gene_id <- mapping_df$gene_id

# Assign seqnames and strand from the exons
transcripts_ranges@seqnames <- seqnames(exons_gtf[match(transcripts_ranges$transcript_id, exons_gtf$transcript_id)])
transcripts_ranges@strand <- strand(exons_gtf[match(transcripts_ranges$transcript_id, exons_gtf$transcript_id)])

# Ensure that seqnames and strands are set correctly
